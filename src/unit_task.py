...
if DBG:
    ...
    from .unit_dataset import UTUTDataset
    from .sequence_generator import SequenceGenerator
...
@dataclass
class UTUTPretrainingConfig(FairseqDataclass):
    data: str = field(
...
@register_task("unit_pretraining", dataclass=UTUTPretrainingConfig)
class UTUTPretrainingTask(FairseqTask):

    cfg: UTUTPretrainingConfig
    ...
    @classmethod
    def setup_task(
        cls, cfg: UTUTPretrainingConfig, **kwargs
    ) -> "UTUTPretrainingTask":
        if cfg.pdb:
            import pdb
            pdb.set_trace()
