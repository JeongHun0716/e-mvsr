...
@register_model("utut_model", dataclass=AVHubertSeq2SeqConfig)
class utut_model(FairseqEncoderDecoderModel):
    def __init__(self, encoder, decoder, tgt_dict, cfg):
        super().__init__(encoder, decoder)
        self.cfg = cfg
        self.masking_updates = cfg.masking_updates
        
        ## progressive masking 
        self.progressive_masking = cfg.progressive_masking
        
        ## utut framework ##
        self.lip_unit_embedding = nn.Embedding(num_embeddings=1004, embedding_dim=1024, padding_idx=0)
        self.speech_unit_embedding = nn.Embedding(num_embeddings=1004, embedding_dim=1024, padding_idx=0)
        self.concat_linear = nn.Linear(2048, 1024)
    ...
    def masking(self, units):
        masking_ratio = self.num_updates / self.masking_updates
        if masking_ratio < 0.1:
            return units
        elif masking_ratio > 0.7:
            return None
        else :
            masking_ratio = (5/3)*(masking_ratio - 0.1)
            B, T = units.size()
            for batch_idx in range(B):
                non_pad_num = sum(units[batch_idx] != 0)
                masking_num = int(masking_ratio * non_pad_num)
                masking_ids = random.sample(range(non_pad_num), masking_num)
                units[batch_idx][masking_ids] = 0 ### Masking -> pading ?
            return units
        
    def forward(self, **kwargs):
        lip_units = kwargs['source']['video'] # B T
        #if self.progressive_masking:   
        speech_units = kwargs['source']['audio'] # B T
        speech_units = self.masking(speech_units)
        
        if speech_units is None:
            speech_units = torch.zeros(lip_units.size(), dtype=int).to(lip_units.device)
        
        lip_units_feat = self.lip_unit_embedding(lip_units) # B T D
        speech_units_feat = self.speech_unit_embedding(speech_units) # B T D
        concat_feat = torch.cat([lip_units_feat, speech_units_feat], dim=-1)
        feats = self.concat_linear(concat_feat)
        
        B, T, D = feats.size()
        lang_feat = self.language_embedding(torch.tensor(kwargs['languages'], dtype=int).to(lip_units.device))        
        lang_feat = repeat(lang_feat, ' b d -> b t d', t=T)
        feats = feats + lang_feat
        
        output = self.encoder.utut_forward(feats, kwargs['padding_mask'])
    ...
    def extract_encoder_output(self, net_input):
        lip_units = net_input['source']['video'] # B T
        #if self.progressive_masking:   
        speech_units = net_input['source']['audio'] # B T
        ...
        B, T, D = feats.size()
        lang_feat = self.language_embedding(torch.tensor(net_input['languages'], dtype=int).to(lip_units.device))        
        lang_feat = repeat(lang_feat, ' b d -> b t d', t=T)
        feats = feats + lang_feat
        
        output = self.encoder.utut_forward(feats, net_input['padding_mask'])
